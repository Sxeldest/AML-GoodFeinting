...
DECL_HOOK(void, CRealTimeShadowManager_DoShadowThisFrame, uintptr_t thiz, ENTITY_TYPE* pPhysical)
{
    if (pPhysical)
    {
        if (pPhysical == (ENTITY_TYPE*)utils::GamePool_FindPlayerPed())
        {
            CRealTimeShadowManager_DoShadowThisFrame(thiz, pPhysical);
            return;
        }

        if (pPhysical->nModelIndex >= 400 && pPhysical->nModelIndex <= 611)
        {
            CRealTimeShadowManager_DoShadowThisFrame(thiz, pPhysical);
            return;
        }
...
DECL_HOOK(void, CShadows_StoreShadowForPedObject, ENTITY_TYPE* pPed, float f1, float f2, float f3, float f4, float f5, float f6)
{
    if (pPed && pPed == (ENTITY_TYPE*)utils::GamePool_FindPlayerPed())
    {
        CShadows_StoreShadowForPedObject(pPed, f1, f2, f3, f4, f5, f6);
    }
}
...
void initializeSAHooks()
{
    ...
    HOOK(SA_Addr(0x5B86B4), CRealTimeShadowManager_DoShadowThisFrame);
    HOOK(SA_Addr(0x5B9E18), CShadows_StoreShadowForPedObject);
