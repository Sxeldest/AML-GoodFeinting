...
void LogVoice(const char* fmt, ...)
{
	...
	const char* pszStorage = (char*) SA_Addr(0x6D687C);
	...
	__android_log_write(ANDROID_LOG_INFO, "Nezuko_SA-MP", buffer);
...
bool Plugin::OnSampLoad() noexcept
{
	...
	SampVoice::AddLoadCallback(Plugin::OnInitGame);
	SampVoice::AddExitCallback(Plugin::OnExitGame);
...
void Plugin::MainLoop()
{
	...
	while (const auto voicePacket = Network::ReceiveVoicePacket()) {
		...
		if (iter == Plugin::streamTable.end()) {
			continue;
		}
...
void Plugin::ControlPacketHandler(const ControlPacket& controlPacket)
{
	switch (controlPacket.packet) {
		...
		case SV::ControlPacketType::addKey: {
			...
			if (controlPacket.length != sizeof(stData)) {
				break;
			}
		...
		case SV::ControlPacketType::createLPStream: {
			...
			LogVoice("[sv:dbg:plugin:createlpstream] : "
					...
					stData.stream, stData.distance, stData.position.X, stData.position.Y,
					stData.position.Z,
		...
		case SV::ControlPacketType::slideStreamParameter: {
			...
			LogVoice("[sv:dbg:plugin:streamslideparameter] : "
					...
					stData.stream, stData.parameter, stData.startvalue, stData.endvalue,
					stData.time);
			...
			iter->second->SlideParameter(stData.parameter, stData.startvalue, stData.endvalue,
					stData.time);
		}
			break;
