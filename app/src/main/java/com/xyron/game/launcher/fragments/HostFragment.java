...
public class HostFragment extends Fragment {
    ...
    private void runHostAction(boolean start) {
        ...
        if (start && (hostRunning || hostStarting)) {
            Toast.makeText(requireContext(), "The host is already on or starting.", Toast.LENGTH_SHORT).show();
            refreshState();
            return;
        }
        if (!start && !hostRunning && !hostStarting) {
            Toast.makeText(requireContext(), "The host is already off.", Toast.LENGTH_SHORT).show();
            refreshState();
            return;
        }
    ...
    private void runAccessHostAction(String accessMode, boolean copyLanAfterStart) {
        ...
        if (hostActionInFlight || remoteTunnelActionInFlight) {
            Toast.makeText(getContext(), "Wait for the current action to finish.", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void copyLanAddress() {
        ...
        if (TextUtils.isEmpty(lanIp)) {
            Toast.makeText(getContext(), "Connect the device to Wi-Fi or hotspot to generate a LAN IP.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (clipboard != null) {
            ...
            Toast.makeText(getContext(), "LAN address copied: " + address, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void startRemoteTunnel() {
        ...
        if (remoteTunnelActionInFlight) {
            Toast.makeText(getContext(), "The remote tunnel is already opening.", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void copyRemoteTunnelAddress() {
        ...
        if (TextUtils.isEmpty(publicUrl)) {
            Toast.makeText(getContext(), "The public address hasn't appeared yet.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (clipboard != null) {
            ...
            Toast.makeText(getContext(), "Tunnel copied: " + publicUrl, Toast.LENGTH_SHORT).show();
        }
    }
