    ...
    public void run() {
        ...
        while (this.b) {
            if (!this.d && i2 > 0) {
                ...
                int iRound = Math.round(AudioTrack.getMinBufferSize(iFmodGetInfo, 12, 2) * 1.1f) & (-4);
                int iFmodGetInfo2 = fmodGetInfo(i);
                int iFmodGetInfo3 = fmodGetInfo(j) * iFmodGetInfo2 * 4;
                AudioTrack audioTrack = new AudioTrack(3, iFmodGetInfo, 12, 2, iFmodGetInfo3 > iRound ? iFmodGetInfo3 : iRound, 1);
                this.c = audioTrack;
